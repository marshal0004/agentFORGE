...
import { useAgentStore, parseFilesFromText, extractPreviewHtml, type ChatMessage } from '../../../stores/agent-store'
        ...
        {messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center gap-5 text-center">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-semibold text-zinc-200">AgentForge</h3>
              <p className="max-w-sm text-sm text-zinc-500 leading-relaxed">
                Describe the app you want to build, and the agent will generate
                production-ready code with all necessary files automatically.
              </p>
            </div>
            <div className="flex flex-col justify-center gap-2 pt-1 w-full max-w-md">
              {[
                { icon: '🎨', text: 'Build a task manager app with CRUD operations' },
                { icon: '📊', text: 'Create a dashboard with charts and analytics' },
                { icon: '📝', text: 'Make a blog with comments and user profiles' },
              ].map((suggestion) => (
                <button
                  key={suggestion.text}
                  onClick={() => setInput(suggestion.text)}
                  className="flex items-center gap-3 rounded-xl border border-zinc-800/50 bg-zinc-900/30 px-4 py-2.5 text-left text-[12px] text-zinc-400 transition-all hover:bg-zinc-800/50 hover:text-zinc-200 hover:border-zinc-700/50"
                >
